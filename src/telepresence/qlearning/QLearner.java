...
public class QLearner implements Constants {
    ...
    /**
     * Get the maximum score of the next possible combinations of actions and
     * states
     *
    ...
    public SelectionResult selectAction(ArrayList<Long> statesActions) {
        ...
        if (r.nextInt(100) < RANDOM_ACTION_PERCENT) {
            ...
            } else // If it has not yet been explored, its score is 0
            {
                maxScore = 0;
            }
        } else {
    ...
    public int learn(int x1, int y1, int x2, int y2, double angle) {
        ...
        while (!state.checkGoal()) {
            ...
            } else // If the goal has been reached, do not calculate the maximum score for new actions
            {
                maxScore = 0;
            }
