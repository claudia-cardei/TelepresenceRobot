...
import telepresence.gui.RobotPanel;
...
/**
 * Reads a map of the environment, builds a path for the robot using BFS and
 * uses the results of Qlearning to determine the robot's moves.
 *
 * @author Filip
 *
 */
public class PathFinder extends Thread implements Constants {
     ...
     * @param mainFrame
     */
    public PathFinder(FloorMap map, RobotPanel robot, MapMarker destination, MainFrame mainFrame) {
        ...
        // Set robot position
        setRobotPosition(robot.getCenterX(), robot.getCenterY(), 270);

        // Set  destination
        this.destination = new Position(destination.getCenterX(), destination.getCenterY());
    ...
    public void readMap(String fileName) {
        ...
        try {
            ...
            for (i = 0; i < n; i++) {
                ...
                for (j = 0; j < m; j++) // 0 is for a free cell, 1 is for an obstacle
                {
                    if (line.charAt(j) == '0') {
                        map[i][j] = 0;
                    } else {
    ...
    /**
     * Check if valid. Used for the BFS path finding algorithm. It checks if
     * there are any obstacles in the window used by the Qlearning algorithm.
     *
    ...
    /**
     * Get the orientation for the window, considering the current and next
     * position
     *
     * @param current the current position
     * @param next the next position
    ...
    /**
     * Get the moves the robot makes in order to reach the first objective,
     * according to the Qlearning results
     *
